...
async function renderMermaid(code, dark) {
  ensureMermaidReady();
  mermaid.initialize({
    htmlLabels: false,
...
async function renderMermaidCached(code, dark) {
  const key = (dark ? "d:" : "l:") + code.length + ":" + code;
  const hit = mermaidCache.get(key);
  if (hit !== undefined) return hit;
  const svg = enhanceSvgVisibility(await renderMermaid(code, dark));
  if (mermaidCache.size > 200) mermaidCache.clear();
  mermaidCache.set(key, svg);
  return svg;
}

// 增强渲染后 SVG 的边缘/箭头可见性（mermaid dark 主题默认浅灰 1px 细线，深色画布上不醒目）。
function enhanceSvgVisibility(svg) {
  if (typeof svg !== "string" || !svg.includes("<svg")) return svg;
  const boost =
    "<style>" +
    "#local-render .edgePath .path{stroke:#7DD3FC!important;stroke-width:2.4px!important}" +
    "#local-render .flowchart-link{stroke:#7DD3FC!important;stroke-width:2.4px!important}" +
    "#local-render .arrowheadPath{fill:#7DD3FC!important}" +
    "#local-render .marker{fill:#7DD3FC!important;stroke:#7DD3FC!important}" +
    "#local-render .edgeLabel{background-color:transparent!important}" +
    "#local-render .edgeLabel p{color:#E5E7EB!important}" +
    "</style>";
  let out = svg;
  ...
  if (req.method === "POST") {
    ...
    readBody(req, cfg.maxBodyBytes)
      .then((raw) => {
        ...
        debugLog("mermaid", "POST", "codeLen=" + code.length, "ver=" + (payload && payload.clientVersion || "?"));
        finish(code, dark);
      })
      .catch((e) => {
