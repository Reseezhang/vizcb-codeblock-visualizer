// vizcb-codeblock-visualizer - node half (v5 / 1.5.0).
...
export function sanitizeSvgText(text) {
  ...
  try {
    ...
    return typeof clean === "string" ? clean : text;
  } catch (e) {
    return text;
  }
}
...
async function handleReadTurn(req, res, sessionQuery, cfg) {
  ...
  debugLog(cfg, "read-turn", "OK", "session=" + sessionId, "seq=" + seq, "ver=" + clientVersion, "blocks=" + out.blocks.length, "reason=" + out.reason, "langs=" + out.blocks.map((bl) => bl.lang).join(","));
...
function handleDebug(res, sessionQuery, cfg) {
  return sendJson(res, 200, {
    ...
    routes: [ROUTE_READ_TURN, ROUTE_MERMAID, ROUTE_DEBUG],
...
export function apply(ctx, config) {
  ...
  console.log(`[vizcb] mounted v${PLUGIN_VERSION}`, JSON.stringify({ maxBlocks: cfg.maxBlocks, mermaidEnabled: cfg.mermaidEnabled, htmlAllowScripts: cfg.htmlAllowScripts, debugLog: cfg.debugLog }));
