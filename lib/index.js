...
const PROMPT_TEXT = [
  ...
  "",
  "输出位置（必须遵守）：",
  "- 代码块必须紧跟在引入它的那句话之后输出，禁止把所有代码块堆积在回答末尾",
  "- 每个代码块之前用一句话说明这张图展示什么，之后继续正文讲解；多个图时图文交替分布",
  "",
  "可用格式：",
  "- ```svg：自包含 SVG 矢量图（首选）",
...
async function handleReveal(req, res, cfg, root) {
  ...
  try {
    let child;
    // explorer 参数坑：`/select,` 必须与路径连写为单个 argv（含空格时引号在内部），
    // 拆成两个 argv 中间插空格会让 Explorer 收不到 ,路径 连写 → 点了没反应。
