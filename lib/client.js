	...
	factory: (require) => {
		...
		const TEXTS = {
			zh: {
				...
				downloadFile: "下载文件",
				openNew: "在新窗口打开",
				srcTruncated: "内容过长，仅显示前 {n} 行，完整内容请下载",
				notice: {
			...
			en: {
				...
				srcTruncated: "Truncated to first {n} lines; download for full content",
				notice: {
		...
		const css = [
			...
			// ── 展示详情（v1.6.9）：tab + 源码 + 下载 + 缩放平移 ──
			".dsh-viz-lb-tabs{display:flex;gap:6px;padding:6px 14px;border-bottom:1px solid var(--dsw-alias-border-l1)}",
			".dsh-viz-lb-tab{font-size:12px;color:var(--dsw-alias-label-secondary);background:transparent;border:1px solid transparent;border-radius:6px;padding:3px 14px;cursor:pointer}",
			".dsh-viz-lb-tab:hover{color:var(--dsw-alias-label-primary)}",
			".dsh-viz-lb-tab-active{color:#e8f0ff;background:rgba(79,140,255,.14);border-color:rgba(79,140,255,.4)}",
			".dsh-viz-lb-stage{position:relative;overflow:auto;flex:1;background:#0F172A;touch-action:pan-x pan-y}",
			".dsh-viz-lb-canvas{position:relative;transform-origin:0 0;touch-action:none}",
			".dsh-viz-lb-src{overflow:auto;flex:1;background:#0F172A}",
			".dsh-viz-lb-src-pre{margin:0;padding:12px;font:13px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace;color:var(--dsw-alias-label-primary);white-space:pre;min-width:max-content}",
			".dsh-viz-lb-src-trunc{padding:6px 14px;font-size:11px;color:var(--dsw-alias-label-tertiary);border-bottom:1px solid var(--dsw-alias-border-l1)}",
			".dsh-viz-lb-foot{display:flex;align-items:center;gap:8px;padding:8px 14px;border-top:1px solid var(--dsw-alias-border-l1)}",
			".dsh-viz-lb-foot .dsh-viz-btn{margin-right:0}",
			// ── present-files 预览 ──
		...
		// ── 展示详情弹窗（v1.6.9）：渲染/源码 tab + 复制/下载/新窗口 + 滚轮缩放/拖拽平移 ──
		const DETAIL_FILE = {
			svg: { ext: ".svg", mime: "image/svg+xml" },
			html: { ext: ".html", mime: "text/html" },
			mermaid: { ext: ".mmd", mime: "text/plain" },
		};
		const SRC_SHOW_LINES = 2000;
		const SRC_MAX_BYTES = 200 * 1024;
		let lastDetailTab = "render"; // 同会话内记住上次 tab（可选）

		function Lightbox(props) {
			const { lang, code, title, t, onClose } = props;
			const [tab, setTab] = react.useState(lastDetailTab);
			const [copied, setCopied] = react.useState(false);
			...
			const maskDownRef = react.useRef(null);
			...
			// 下载原始源码（模型写的原文，不是消毒产物）；文件名 = 标题 + 类型扩展名。
			const onDownload = () => {
				const f = DETAIL_FILE[lang] || { ext: ".txt", mime: "text/plain" };
				const blob = new Blob([code], { type: f.mime + (f.mime.indexOf("text") === 0 ? ";charset=utf-8" : "") });
				fallbackDownload(safeFileName(title) + f.ext, blob);
			};
			const onOpenNew = () => {
				const f = DETAIL_FILE[lang];
				if (!f) return;
				const blob = new Blob([code], { type: f.mime + (f.mime.indexOf("text") === 0 ? ";charset=utf-8" : "") });
				const url = URL.createObjectURL(blob);
				const w = window.open(url, "_blank");
				if (w) w.focus();
				window.setTimeout(() => URL.revokeObjectURL(url), 30000); // 给新窗口加载时间再回收
			};
			const setTabSafe = (v) => { lastDetailTab = v; setTab(v); };

			// ESC 关闭 + 焦点管理（打开聚焦关闭键，关闭还给触发元素）
			react.useEffect(() => {
				const onKey = (e) => { if (e.key === "Escape") onClose(); };
				window.addEventListener("keydown", onKey);
				const prev = document.activeElement;
				const raf = window.requestAnimationFrame(() => { if (closeRef.current) closeRef.current.focus(); });
				return () => {
			...
			// 滚轮缩放（仅渲染 tab；passive:false 才能 preventDefault）
			react.useEffect(() => {
				const stage = stageRef.current;
				if (!stage || tab !== "render") return;
				const onWheel = (e) => {
					e.preventDefault();
			...
			const onCanvasPointerEnd = (e) => {
				if (dragRef.current && dragRef.current.id === e.pointerId) dragRef.current = null;
			};
			// 遮罩关闭：若按下与抬起位置差太多（拖动过）则不关闭（规格 §7）
			const onMaskPointerDown = (e) => { maskDownRef.current = { x: e.clientX, y: e.clientY }; };
			const onMaskClick = (e) => {
				const d = maskDownRef.current;
			...
			let renderBody;
			if (lang === "mermaid") {
				renderBody = react.createElement(MermaidBlock, { code: code, t: t, className: "dsh-viz-lb-svg" });
			} else if (lang === "html") {
				renderBody = react.createElement("iframe", { className: "dsh-viz-lb-frame", sandbox: CFG.htmlAllowScripts ? "allow-scripts" : "", srcDoc: htmlDoc(code), title: "HTML" });
			} else {
			...
			// 源码区：原始文本（textContent 直出，不解析为 HTML）；超长截断（下载仍是完整内容）
			const truncated = code.length > SRC_MAX_BYTES;
			const shown = truncated ? code.split("\n").slice(0, SRC_SHOW_LINES).join("\n") : code;

			return react.createElement("div", { className: "dsh-viz-lightbox", role: "dialog", "aria-modal": "true", onPointerDown: onMaskPointerDown, onClick: onMaskClick },
				react.createElement("div", { className: "dsh-viz-lb-box", onClick: (e) => e.stopPropagation() },
					react.createElement("div", { className: "dsh-viz-lb-head" },
						react.createElement("span", { className: "dsh-viz-lb-title", title: title }, title),
						react.createElement("div", { className: "dsh-viz-lb-actions" },
							react.createElement("button", { ref: closeRef, type: "button", className: "dsh-viz-btn", onClick: onClose }, t.close),
						),
					),
					react.createElement("div", { className: "dsh-viz-lb-tabs" },
						react.createElement("button", { type: "button", className: "dsh-viz-lb-tab" + (tab === "render" ? " dsh-viz-lb-tab-active" : ""), onClick: () => setTabSafe("render") }, t.detailRender),
						react.createElement("button", { type: "button", className: "dsh-viz-lb-tab" + (tab === "source" ? " dsh-viz-lb-tab-active" : ""), onClick: () => setTabSafe("source") }, t.detailSource),
					),
					tab === "render"
						? react.createElement("div", { className: "dsh-viz-lb-stage", ref: stageRef },
							react.createElement("div", {
								className: "dsh-viz-lb-canvas",
						...
						: react.createElement("div", { className: "dsh-viz-lb-src" },
							truncated ? react.createElement("div", { className: "dsh-viz-lb-src-trunc" }, t.srcTruncated.replace("{n}", String(SRC_SHOW_LINES))) : null,
							react.createElement("pre", { className: "dsh-viz-lb-src-pre" }, shown),
						),
					react.createElement("div", { className: "dsh-viz-lb-foot" },
						react.createElement("button", { type: "button", className: "dsh-viz-btn", onClick: onCopy }, copied ? t.copied : t.copy),
						react.createElement("button", { type: "button", className: "dsh-viz-btn", onClick: onDownload }, t.downloadFile),
						(lang === "svg" || lang === "html")
							? react.createElement("button", { type: "button", className: "dsh-viz-btn", onClick: onOpenNew }, t.openNew)
							: null,
					),
				),
			);
