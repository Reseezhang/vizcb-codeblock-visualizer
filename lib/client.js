...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// hideSourceBlocks 兜底：CSS :has 先行隐藏核心语言围栏，JS 再按解析结果精确隐藏
			"pre:has(> code.language-svg, > code.language-html, > code.language-mermaid, > code.language-present-files){display:none!important}",
		...
		function VizTail(props) {
			...
			react.useEffect(() => {
				...
				while (container && container.parentElement && !container.querySelector("pre, code, [data-lang]")) container = container.parentElement;
